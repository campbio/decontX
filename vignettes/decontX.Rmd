...
**Note on reproducing results from older versions:** As of decontX 1.11.1, the heuristic clustering used when `z` is not supplied (log-normalization, variable gene selection, PCA, and UMAP) is performed with the [scrapper](https://bioconductor.org/packages/scrapper/) package instead of scater/scuttle, whose normalization functions were deprecated in Bioconductor 3.24. The workflow and parameters are unchanged, but contamination estimates may differ slightly from versions 1.11.0 and earlier because the UMAP embedding used for the initial clustering differs. To reproduce results from older versions, set `legacyInit = TRUE`. This option requires the scater package and is kept for backwards compatibility; it will remain available as long as the underlying functions are still provided by scater/scuttle. Results are identical between versions when cluster labels are supplied with `z`.
...
plotReducedDim(sce, dimred = "decontX_UMAP",
    colour_by = "decontX_clusters", text_by = "decontX_clusters")
```
...
# Load scrapper after scater so that normalizeCounts() below refers to
# scrapper's function and not the deprecated scuttle version that scater
# attaches.
library(Matrix)
library(scrapper)
library(patchwork)
logcounts(sce) <- normalizeCounts(counts(sce),
    size.factors = centerSizeFactors(colSums(counts(sce))))
markers <- c("CD3D", "CD3E", "GNLY",
    "LYZ", "S100A8", "S100A9",
    "CD79A", "CD79B", "MS4A1")
wrap_plots(
    lapply(markers, function(g) {
        plotReducedDim(sce, dimred = "decontX_UMAP", colour_by = g)
    }),
    ncol = 3)
```
...
To label the plots by cell type instead of by cluster number, the barplot
functions take a `groupClusters` argument: a named list that maps each cell
type to the cluster number(s) it corresponds to. The cluster numbers assigned
by the heuristic clustering can differ between decontX versions and platforms,
so here we let decontX figure out the mapping for us. The resulting
`cellTypeMappings` looks like this:

```{r deriveMapping, echo=FALSE}
# The cluster numbers are not stable across decontX versions/platforms, so
# rather than hardcoding them we assign each cell type to the cluster in which
# its marker genes are detected most frequently.
cellTypeMappings <- lapply(markers, function(genes) {
    detectionRate <- tapply(colMeans(counts(sce)[genes, , drop = FALSE] > 0),
        sce$decontX_clusters, mean)
    as.integer(names(which.max(detectionRate)))
})
names(cellTypeMappings) <- c("Tcells", "Bcells", "Monocytes", "NKcells")
```
...
You can also build this list by hand: inspect `sce$decontX_clusters`, decide
which cluster corresponds to each cell type, and write it out, e.g.
`cellTypeMappings <- list(Tcells = 2, Bcells = 5, Monocytes = 1, NKcells = 6)`
(your cluster numbers may differ). A single cell type can also span several
clusters — just give it a vector of cluster numbers, for example
`cellTypeMappings <- list(Tcells = c(2, 4), Bcells = 5, Monocytes = 1, NKcells = 6)`
groups clusters 2 and 4 together as T-cells.
...
assay(sce, "decontXlogcounts") <- normalizeCounts(
    decontXcounts(sce),
    size.factors = centerSizeFactors(colSums(decontXcounts(sce))))
