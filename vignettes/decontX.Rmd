...
logcounts(sce) <- scrapper::normalizeCounts(counts(sce),
    size.factors = scrapper::centerSizeFactors(Matrix::colSums(counts(sce))))
...
The cluster numbers produced by the heuristic clustering can change
between decontX versions and platforms, so instead of hardcoding the
cluster-to-cell-type mapping we derive it programmatically by assigning
each cell type to the cluster in which its markers are detected most
frequently:
...
markers <- list(Tcell_Markers = c("CD3E", "CD3D"),
    ...
    NKcell_Markers = "GNLY")
cellTypeMappings <- lapply(markers, function(g) {
    detected <- Matrix::colMeans(counts(sce)[g, , drop = FALSE] > 0)
    perCluster <- tapply(detected, sce$decontX_clusters, mean)
    as.integer(names(which.max(perCluster)))
})
names(cellTypeMappings) <- c("Tcells", "Bcells", "Monocytes", "NKcells")
unlist(cellTypeMappings)
...
assay(sce, "decontXlogcounts") <- scrapper::normalizeCounts(
    decontXcounts(sce),
    size.factors = scrapper::centerSizeFactors(
        Matrix::colSums(decontXcounts(sce))))
